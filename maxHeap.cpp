//maxHeap member function definition
#include "maxHeap.h"
MaxHeap::MaxHeap()
{
...
void MaxHeap::push(int new_element)
{
	//add a new leaf to heap
	int dummy;
	elements.push_back(dummy);
	int index = elements.size() - 1;
	//demote (level down the position) parents that are smaller than the new element
...
void MaxHeap::pop()
{
	//to remove the max value (elements[1]) in the heap, we need to assign the max_index (which is elements[1])
...
void MaxHeap::fix_heap()//to reheap the heap, make sure the root is the max
{
	int root = elements[1];
	int last_index = elements.size() - 1;
	int index = 1;
	...
	while(!done)
	{
		...
		//but before comparing, we need to know if the right child is greater than left child
		...
		if(child_index <= last_index)
		{
			...
			{
				//if root is larger than both child, end the loop
				done = true;
			}
		}
		else 
...
int MaxHeap::get_left_child_index(int index)
{
	return 2*index;
}
int MaxHeap::get_right_child_index(int index)
{
	return 2*index + 1;
}
int MaxHeap::get_parent_index(int index)
{
	return index / 2;
}
